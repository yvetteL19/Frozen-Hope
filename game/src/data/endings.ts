...
几个小时后，直升机的轰鸣声穿透了风雪。
当舱门打开，救援队员跳下来时，${survivors.find((npc) => npc.roleId === 'assistant') ? '助理哭了出来。' : '幸存者们喜极而泣。'}
...
`;
      ...
      if (survivors.length > 0) {
        description += `你回头看看${survivors.map((npc) => npc.name).join('、')}。
他们脸上的表情和你一样——劫后余生的麻木。
...
`;
      ...
      if (dead.length > 0) {
        description += `没能活下来的人：
${dead.map((npc) => `- ${npc.name} (${npc.roleId === 'pilot' ? '飞行员' : npc.roleId === 'ceo' ? 'CEO' : npc.roleId === 'assistant' ? '助理' : npc.roleId === 'guide' ? '向导' : npc.roleId === 'sales' ? '销售总监' : '程序员'})`).join('\n')}
...
你回头看看${survivors.map((npc) => npc.name).join('、')}。
所有人都还活着。
...
你最后的意识，是${survivors.length > 0 ? `${survivors[0].name}的呼喊声："醒醒！别睡！"` : '风雪的呼啸声'}。
